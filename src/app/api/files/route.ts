...
export async function GET(request: Request) {
  ...
  const parsedKvFiles = kvFiles.map((f: any) => typeof f === 'string' ? JSON.parse(f) : f);

  // 2. 로컬(content 폴더) 파일 목록 가져오기
  // agentId 매핑 (real-estate -> realestate, beramode -> veramode 등)
  ...
  if (fs.existsSync(contentPath)) {
    ...
    for (const filename of filenames) {
      if (filename.endsWith('.md')) {
        ...
        localFiles.push({
          name: filename,
          url: `/api/content?path=${folderName}/${filename}`, // 컨텐츠 전용 API
          size: stats.size,
